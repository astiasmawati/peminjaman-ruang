...
class Peminjam extends CI_Controller
{
	...
	public function batalpinjam($id_user, $id_perangkat)
	{
		$batalpinjam = $this->m_siplabs->deletedata('peminjaman', array('id_user'=>$id_user, 'id_perangkat'=>$id_perangkat));
		if ($batalpinjam) {
			$this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissible">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
	...
	public function jadwal()
	{
		...
		$data['jadwal'] = $this->db->query("SELECT * FROM jadwal 
			INNER JOIN peminjaman, ruangan, user 
